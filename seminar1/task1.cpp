...
// Generic recursive quick sort
void quick_sort_rec(std::vector<int>& a, int low, int high, PivotFunc pickPivot) {
  if (low < high) {
    int pivotIndex = pickPivot(a, low, high);
    int p = partition(a, low, high, pivotIndex);

    quick_sort_rec(a, low, p - 1, pickPivot);
    quick_sort_rec(a, p + 1, high, pickPivot);
  }
}

// Recursive wrappers
void quick_sort_start(std::vector<int>& vec) {
  quick_sort_rec(vec, 0, vec.size() - 1, pivot_start);
}

void quick_sort_rand(std::vector<int>& vec) {
  quick_sort_rec(vec, 0, vec.size() - 1, pivot_rand);
}

void quick_sort_m3(std::vector<int>& vec) {
  quick_sort_rec(vec, 0, vec.size() - 1, pivot_m3);
}
